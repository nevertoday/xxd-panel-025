<p align="center">
  <img src="./assets/banner.svg" alt="XXD Panel 025 项目横幅" width="1200">
</p>
...
[![Four Modes](https://img.shields.io/badge/Modes-4-d75d32?style=flat-square)](#四种输出共享同一种双重阅读逻辑)
...
> 一眼主体 · 第二眼隐藏意象 · 正负形反转 · 2–4 色莫兰迪 · 真实丝网触感

XXD Panel 025 是一个面向 Codex 与兼容 Agent 的图像生成 Skill。它从照片里锁定一个清晰、可辨认的主体剪影，再从同一来源中选择一个最有依据的环境、结构、物件或象征意象，让两者通过轮廓咬合、负形切口、共享边界与图底反转合成一个视觉锚点：远看先见主体，近看才发现隐藏意象。
...
```text
锁定源图主体 → 提炼可辨认剪影 → 从同一来源选择隐藏意象 → 让轮廓与负形真正咬合 → 建立远看／近看的阅读次序 → 提炼 2–4 色莫兰迪色块 → 加入克制的物理丝网痕迹 → 把文字嵌入共享边界
```

如果换成一张无关照片，主体剪影、隐藏意象、共享轮廓、综合色温和文字仍然成立，这张图就不属于 025。

## 025 的视觉契约

- **主体先读：** 至少三个源图专属线索保住身份、轮廓、姿态、动作与关系，缩略图中仍能一眼辨认。
- **隐藏意象后读：** 只选择一个与同一源图紧密相关的环境、结构、物件或象征意象，近看才被发现。
- **真正合成一个图形：** 使用轮廓咬合、负形切口、共享边界与图底反转；禁止并排双物、透明叠加和普通双重曝光。
- **一个视觉锚点：** 两种阅读共享同一构图重心，浅色纸张留白为发现过程提供呼吸。
...
项目已预留 [`assets/examples/`](assets/examples/) 样张目录。只有经项目作者确认、确实使用 025 完成的作品才会加入；在此之前不借用其他风格的推文或图片作为占位。

未来样张只用于展示 025 对不同题材的适应力，不会把样张主体、隐喻、配色、文案或画幅变成生成参考或默认值。
...
| 模式 | 尺寸逻辑 | 成品 |
| --- | --- | --- |
| `top-bottom` | 源图自适应 | 上方完整原图，下方 025 格式塔莫兰迪丝网版画；两块都保持原图完整尺寸，严格 50/50 |
| `left-right` | 源图自适应 | 左侧完整原图，右侧 025 格式塔莫兰迪丝网版画；两块都保持原图完整尺寸，严格 50/50 |
| `design-only` | 源图自适应 | 只显示变化设计，不显示原照片；沿用原图比例和尺寸 |
| `wallpaper-pack` | 四种设备尺寸 | 分别输出手机、iPad、电脑、儿童手表四张 PNG |

用户精确尺寸 > 指定比例或用途 > 普通模式源图自适应。原始 `025.md` 里的 3:4 只是一开始的创作画幅，不会被写成当前 Skill 的静默默认值。
...
- **四张独立：** 每张只参考原照片，可以分别探索不同的主体／隐藏意象配对、共享轮廓、莫兰迪配色与文字介入。
...
自动文案从照片可见或有依据的主题、动作、关系、环境与第二重意象中提炼一个极短标题。它应让主体与隐藏图形在语义上扣合，看到第二层后产生“原来如此”的会心一击，而不是泛泛的情绪口号。
...
图像模型负责源图绑定的主体剪影、隐藏意象、共享边界、图底反转、莫兰迪色块、物理丝网质感以及融入负形的文字。`scripts/compose_panel.py` 只负责画布规划、精确 50/50 位图拼合、最终尺寸和审计，不会用程序绘图伪造成品。
...
- [Skill 工作流](SKILL.md)
- [中文完整提示词](references/xxd-panel-025-prompt.zh-CN.md)
- [英文完整提示词](references/xxd-panel-025-prompt.en.md)
- [原始风格提示词](references/025-source.md)
