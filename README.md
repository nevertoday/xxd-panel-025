...
| 模式 | 画幅逻辑 | 成品 |
| --- | --- | --- |
| `top-bottom` | 用户确认的整张成品画幅 | 一次生成完整画布：高保真原图在上，025 设计在下，约 50/50 |
| `left-right` | 用户确认的整张成品画幅 | 一次生成完整画布：高保真原图在左，025 设计在右，约 50/50 |
| `design-only` | 用户确认的整张成品画幅 | 025 设计铺满画布，不显示原照片 |
...
```text
/xxd-panel-025 photo.jpg --mode top-bottom,design-only --size auto,3:4,9:16 --text auto --locale ja-JP
```
