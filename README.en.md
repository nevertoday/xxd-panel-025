...
The repository reserves [`assets/examples/`](assets/examples/) for future work. Only finished 025 artwork verified by the project owner will be added; no post or image from another style is used as a placeholder.

Future samples will demonstrate 025's adaptability. Their subjects, metaphors, palette, copy, and canvas ratios will never become generation references or defaults.
...
| --- | --- | --- |
| `top-bottom` | source-adaptive | complete source above, 025 Gestalt Morandi screenprint below; both panels retain the source size and split exactly 50/50 |
| `left-right` | source-adaptive | complete source left, 025 Gestalt Morandi screenprint right; both panels retain the source size and split exactly 50/50 |
...
Exact user pixels > explicit ratio or destination > source adaptation for ordinary modes. The original `025.md` used a 3:4 creative canvas, but that historical example is not a silent default in the current Skill.
...
- **Independent set:** every device references only the source photograph and may explore a different subject/hidden-image pairing, shared contour, Morandi palette, and type intervention.
...
## Copy must belong to the double reading and print structure
...
Automatic copy distils one extremely short title from the photograph's supported subject, action, relation, environment, and second image. The words should bind both readings so the hidden shape produces an “of course” moment, rather than a generic mood slogan.
...
The image model creates the source-bound principal silhouette, hidden image, shared boundary, figure–ground reversal, Morandi colour shapes, physical screenprint evidence, and type integrated with negative space. `scripts/compose_panel.py` only plans canvases, performs exact 50/50 raster composition, finalises dimensions, and audits results. It never fakes artwork with programmatic drawing.
...
- [Chinese full prompt](references/xxd-panel-025-prompt.zh-CN.md)
- [English full prompt](references/xxd-panel-025-prompt.en.md)
